...
function* cycle10_6() {
	...
	do {
		...
		while (
			...
			// else
			if (core.PC === tempBP || breakpoints.has(core.PC)) {
...
function _recordCycle() {
	...
	_cycleLog += `${core.PC.toString(16).toUpperCase().padStart(4, "0")}  `;
	...
	_cycleLog += `core.SP:${core.SP.toString(16).toUpperCase().padStart(2, "0")} `;
	if (core.calcAddress === -1) _cycleLog += "                 ";
	else {
		_cycleLog += `CA:${core.calcAddress.toString(16).toUpperCase().padStart(4, "0")} `;
		_cycleLog += `(${(CalcBanked >> 16).toString(16).toUpperCase().padStart(2, "0")}:`;
		_cycleLog += `${(CalcBanked & 0xffff).toString(16).toUpperCase().padStart(4, "0")})`;
	}
	_cycleLog += ` ${core.cycle_count}`;
	_cycleLog += "\n";
	if (op === 0) _cycleLog += "\n";
}
