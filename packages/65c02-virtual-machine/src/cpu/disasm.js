...
function readbyte(_bank, addr, readingOpcode = false) {
	return core.bus.read(addr, !readingOpcode);
	// addr&= 0xFFFF;
	// return this.memory[bank*0x10000+addr];
}

function readword(_bank, addr, readingOpcode = false) {
...
function getPrevInstrAddr(bank, addr) {
	let address = addr;
	address &= 0xffff;
	...
	for (let startingPoint = address - 20; startingPoint !== address; startingPoint++) {
		...
		while (startAddr < address) {
			const result = disassemble(bank, startAddr);
