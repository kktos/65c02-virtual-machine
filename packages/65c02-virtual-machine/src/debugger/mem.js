...
export default class MemViewer {
	constructor(dbg, elm, count) {
		...
		elm.addEventListener("wheel", (e) => this.onMouseWheel(e), {
			passive: true,
		});
	...
	async onMouseClick(e) {
		...
		if (e.target.className === "value") {
			...
			const bank = Number.parseInt(`0x${parts[0]}`, 10);
			const addr = Number.parseInt(`0x${parts[1]}`, 10) + Number(e.target.id);

			// this.dbg.getValueDlg();

			value = Number.parseInt(prompt(`${utils.hexbyte(bank)}:${utils.hexword(addr)}: Hexa Value ?`), 16);
			if (Number.isNaN(value)) return;

			// this.memory[bank*0x10000 + addr]= value & 0xFF;
			await this.vm.waitMessage("memWrite", {
				addr: (bank << 16) + addr,
				value: value & 0xff,
			});
	...
	async update() {
		const bytes = await this.vm.waitMessage(this.wannaRealAddr ? "dbgReadBytes" : "memReadBytes", {
			addr: (this.dumpMemBank << 16) + this.dumpMemAddr,
			count: 640,
		});
		let dumpStr = "";
		for (let line = 0; line < this.lineCount; line++) {
			...
			for (let column = 0; column < 16; column++) {
				...
				byte &= 0x7f;
				if (byte < 0x20) charsStr += ".";
				else charsStr += String.fromCharCode(byte);
			}
