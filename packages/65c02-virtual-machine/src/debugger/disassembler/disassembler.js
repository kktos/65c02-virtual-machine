...
export default class Disassembler {
	...
	async readbyte(_bank, addr) {
		return await this.vm.waitMessage("dbgReadBytes", {
			addr: addr & 0xffff,
			count: 1,
		});
		// addr&= 0xFFFF;
		...
		for (let i = 0; i < temp_str.length; i++) {
			switch (temp_str[i]) {
				...
				case "r": {
					...
					if (willBranch) comment = "will branch";
					break;
				}
				...
				case "%": {
					...
					switch (addrMode) {
						case "(%)": {
							finalAddr = utils.hexword(
								(await this.readbyte(bank, destAddr)) | ((await this.readbyte(bank, destAddr + 1)) << 8),
							);
							break;
						}
						case "%": {
	...
	_disassemble(addr) {
		...
		switch (param) {
			...
			case "zp": {
				...
				return [`${split[0]} $${utils.hexbyte(this.readbyte(bank, addr + 1))}${suffix}; $${zpValue}`, addr + 2];
			}
			case "(,x)":
			...
			case "()":
				...
				return [
					`${split[0]} ($${utils.hexbyte(this.readbyte(bank, addr + 1))})${suffix} ; $${utils.hexword(destAddr)}${suffix2}`,
					addr + 2,
				];
			case "(abs)":
				destAddr = this.readbyte(bank, addr + 1) | (this.readbyte(bank, addr + 2) << 8);
