		...
		switch (newAddr) {
			...
			case 0xc030:
				this.controller.postMessage({
					cmd: "sound",
					data: { mode: "tick", cycles: this.controller.core.cycle_count },
				});
				return;
	...
	write(addr, value) {
		...
		if (newAddr < 0xc000) {
			...
			if (newAddr < 0x0800 && newAddr >= 0x0400) {
				...
				this.controller.postMessage({
					cmd: "video",
					data: { mode: "mem", bank, addr: newAddr },
				});
				return;
			}
		...
		switch (newAddr) {
			...
			case SWITCHES.SLOT3_0E:
				this.controller.postMessage({
					cmd: "video",
					data: { mode: "ctrl", addr: newAddr, value },
				});
				break;
			...
			case SWITCHES.ALTCHARSETOFF:
				...
				this.controller.postMessage({
					cmd: "video",
					data: { mode: "altCharset", value: this.altCharsetOn },
				});
				break;
