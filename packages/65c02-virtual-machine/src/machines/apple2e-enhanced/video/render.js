...
export default class Video {
	...
	handleMessage(sender, msg) {
		...
		if (msg.update !== undefined) {
			this.cacheHGRstate[msg.update] = true;
			return this.cacheHGRstate[msg.update];
		}
	...
	renderHighGraphic(ctx) {
		this.cacheHGR.forEach((img, part) => {
			...
			if (img) ctx.drawImage(img, 0, part * 155, 700, 155);
		});

		if (this.mixed) this.renderText40Mixed(ctx);
	}
	...
	renderHighGraphic_old(mainCtx) {
		...
		if (this.mixed) this.renderText40Mixed(mainCtx);
