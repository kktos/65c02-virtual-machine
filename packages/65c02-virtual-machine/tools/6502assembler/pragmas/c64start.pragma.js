...
export function c64Start(ctx, pragma) {
	...
	let remText = "";
	// lineLengthMax=88,
	const lineNumberMax = "63999";
	const basicAddr = pragma === "PETSTART" ? 0x0401 : 0x0801;
	const rem = [];
	let linkAddr;
	...
	if (remText) {
		...
		let cnt = 0;
		let t = [];
		for (let i = 0; i < remText.length; i++) {
			const c = remText.charAt(i);
			let cc = remText.charCodeAt(i);
			ctx.pict += c;
		...
		ctx.pict += '"';
		if (Number.parseInt(basicLineNo, 10) < rem.length) basicLineNo = `${rem.length}`;
	}
	logLine(ctx);
	if (ctx.pass === 2) ctx.listing += ">>>>  COMPILING BASIC PREAMBLE...\n";
	if (rem.length) {
		for (let ln = 0; ln < rem.length; ln++) {
			const remLine = rem[ln];
			linkAddr = ctx.pc + 7 + remLine.length;
			if (ctx.pass === 2) {
				const linkLo = linkAddr & 0xff;
				const linkHi = linkAddr >> 8;
				(lnLo = ln & 0xff), (lnHi = ln >> 8), (ctx.addrStr = getHexWord(ctx.pc));
				...
				ctx.asm = "";
				ctx.pict = ";";
				for (let i = 0; i < remLine.length; i++) {
	...
	if (ctx.pass === 2) {
		const linkLo = linkAddr & 0xff;
		const linkHi = linkAddr >> 8;
		const ln = Number.parseInt(basicLineNo, 10);
		const lnLo = ln & 0xff;
		const lnHi = ln >> 8;
		const saStr = `${ctx.cbmStartAddr}`;
		ctx.addrStr = getHexWord(ctx.pc);
		compile(ctx, ctx.pc++, linkLo);
		compile(ctx, ctx.pc++, linkHi);
		ctx.asm = `${getHexByte(linkLo)} ${getHexByte(linkHi)}`;
		...
		ctx.asm = "";
		ctx.pict = ';TEXT "';
		for (let i = 0, max = saStr.length - 1; i <= max; i++) {
			const c = saStr.charAt(i);
			const cc = saStr.charCodeAt(i);
	...
	if (ctx.pass === 2) ctx.listing += `>>>>  START OF ASSEMBLY AT $${getHexWord(ctx.pc)} ("SYS ${ctx.cbmStartAddr}")\n`;
