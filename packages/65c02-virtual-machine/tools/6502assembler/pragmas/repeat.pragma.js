...
function repeatNextLine(ctx, repeatCtx) {
	if (repeatCtx.lineIdx >= repeatCtx.lines.length) {
		...
		if (!repeatCtx.times) return false;
		repeatCtx.lineIdx = 0;
	}
...
export function processRepeat(ctx, pragma) {
	...
	if (ctx.ofs + 1 < ctx.sym.length) {
		...
		if (getNSentry(ctx, "%locals%").v == null) getNSentry(ctx, "%locals%").v = [];
		getNSentry(ctx, "%locals%").v.push(iterator);
		iterator.name = name;
	}

	if (ctx.pass === 1) logLine(ctx);
	...
	if (repeatCtx.lines) {
		if (ctx.pass === 2 && repeatCtx.times > 0) registerNextLineHandler(pragma, (ctx) => repeatNextLine(ctx, repeatCtx));
		return true;
	}
