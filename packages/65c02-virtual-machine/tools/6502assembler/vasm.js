...
assemble(basename(filename), opts).then((ctx) => {
	if (ctx.error) {
	...
	Object.keys(ctx.segments).forEach((segmentName) => {
		if (ctx.code[segmentName]) finalCode = finalCode.concat(ctx.code[segmentName]);
		...
		console.log(
			...
			"len",
			`$${getHexWord(segment.end - segment.start + 1)}`,
		);
		if (argv.dump) {
	...
	if (argv.symbols === true) console.log(`SYMBOLS:\n${dumpSymbols().join("\n")}`);
