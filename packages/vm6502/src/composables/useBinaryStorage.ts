export function useBinaryStorage() {
	const DB_NAME = "vm6502_binary_storage";
	...
	const saveBinary = async (key: IDBValidKey, name: string, data: ArrayBuffer) => {
	...
	const loadBinary = async (key: IDBValidKey) => {
		const db = await getDb();
	...
	const deleteBinary = async (key: IDBValidKey) => {
		const db = await getDb();
		return new Promise<void>((resolve, reject) => {
			const transaction = db.transaction(STORE_NAME, "readwrite");
			const store = transaction.objectStore(STORE_NAME);
	...
	return { saveBinary, loadBinary, getAllBinaries, deleteBinary };
}
