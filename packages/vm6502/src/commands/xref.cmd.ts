...
// Opcodes that constitute a "Call" or "Jump" flow transfer (excluding Indirect JMP which is a read)
const CALL_OPS = new Set([0x20, 0x4c]);
const ACCESS_OPS = Object.keys(ABS_OPCODES)
	.map(Number)
	.filter((op) => !CALL_OPS.has(op));

export const xrefCmd: Command = {
	description:
	...
	fn: ({ vm, params }: CommandContext): CommandResult => {
		...
		const results: { address: number; type: string; instruction: string }[] = [];
		const read = (a: number) => vm.readDebug(a) ?? 0;

		// 1. Branches (Relative)
		// We scan a window around the target address because branches have a limited range (-128 to +127).
		...
		// We scan the entire memory for opcodes targeting the address.
		if (filter !== "BRANCH") {
			const targetLo = targetAddr & 0xff;
			const targetHi = (targetAddr >> 8) & 0xff;

			// Prepare candidates based on filter
