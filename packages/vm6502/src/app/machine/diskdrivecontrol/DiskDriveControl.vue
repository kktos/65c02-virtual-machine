	...
	<div class="flex items-center space-x-3 bg-gray-800 p-2 rounded-lg shadow-md border border-gray-700">
		...
		<Popover v-model:open="isLibraryOpen">
			<PopoverTrigger as-child>
				<button
					class="group relative flex items-center justify-center w-10 h-10 bg-gray-700 hover:bg-gray-600 rounded border border-gray-600 transition-colors shrink-0"
					title="Disk Library"
				>
					<svg
						xmlns="http://www.w3.org/2000/svg"
						class="h-5 w-5 text-yellow-400 hover:text-yellow-300"
						fill="none"
						viewBox="0 0 24 24"
						stroke="currentColor"
					>
					...
					<span
						v-if="loggingEnabled"
						class="absolute top-1 right-1 w-2 h-2 bg-red-500 rounded-full animate-pulse shadow-[0_0_4px_rgba(239,68,68,0.6)]"
					></span>
				</button>
			</PopoverTrigger>
			<PopoverContent class="bg-gray-900 border-gray-700 text-gray-100 w-[340px] p-0" align="center">
				<div class="p-4 pb-2 border-b border-gray-800">
					...
					<p class="text-xs text-gray-400 mt-1">Manage disks and drive settings.</p>
				</div>
				...
				<ScrollArea class="h-[300px] p-4">
					<div class="space-y-2">
						...
						<div
							...
							:key="disk.key.toString()"
							class="group relative flex items-center justify-between p-2 bg-gray-800 rounded border border-gray-700 hover:border-blue-800/50 transition-colors"
						>
							...
							<button
								@click.stop="handleDelete(disk.key)"
				...
				<div class="p-4 border-t border-gray-800 bg-gray-900/50 flex flex-col gap-3">
					<Tabs default-value="url" class="w-full">
						...
						<TabsContent value="url" class="mt-3 space-y-2">
							...
							<div v-if="urlError" class="text-red-400 text-[10px] px-1">
								{{ urlError }}
							</div>
						</TabsContent>
					...
					<div class="flex items-center justify-between">
						<label class="flex items-center space-x-2 text-sm text-gray-300 cursor-pointer select-none">
							<input
								...
								class="rounded bg-gray-800 border-gray-600 text-blue-500 focus:ring-blue-500 focus:ring-offset-gray-900"
							/>
							<span>Enable Logging</span>
						</label>
						<button
							@click="openLogs"
							class="text-xs text-blue-400 hover:text-blue-300 flex items-center px-2 py-1 hover:bg-gray-800 rounded transition-colors"
						>
