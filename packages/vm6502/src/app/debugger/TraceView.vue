<template>
	<div class="p-4 bg-gray-800 rounded-lg shadow-xl h-full flex flex-col">
		<div class="flex justify-between items-center mb-3 border-b border-gray-700/50 pb-2 shrink-0">
			<h2 class="text-sm font-semibold text-cyan-400 uppercase tracking-wider">Execution Trace</h2>
			<div class="flex space-x-2">
				<Popover>
					...
					<PopoverContent class="w-60 bg-gray-800 border-gray-700 text-gray-100 p-3" align="end">
						<div class="flex flex-col space-y-2">
							...
							<input
								type="number"
								v-model.number="traceSize"
								class="bg-gray-900 border border-gray-700 rounded px-2 py-1 text-xs text-white focus:outline-none focus:border-cyan-500"
								min="10"
								max="1000"
								step="10"
							/>
				...
				<button @click="refreshTrace" class="p-1 rounded text-gray-400 hover:text-gray-200 hover:bg-gray-700 transition-colors" title="Refresh Trace">
					<svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" viewBox="0 0 20 20" fill="currentColor">
						<path
							fill-rule="evenodd"
				...
				<button @click="clearTrace" class="p-1 rounded text-gray-400 hover:text-red-400 hover:bg-gray-700 transition-colors" title="Clear Trace">
					<svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" viewBox="0 0 20 20" fill="currentColor">
						<path
							fill-rule="evenodd"
			...
			<table class="text-left border-collapse w-full">
				...
				<tbody>
					...
					<tr v-if="traceHistory.length === 0">
						<td colspan="3" class="py-4 text-center text-gray-600 italic">No trace data. Enable tracing in controls.</td>
					</tr>
				</tbody>
			</table>
		</div>
...
onUnmounted(() => {
	if (vm?.value) vm.value.onTraceReceived = undefined;
});
</script>
