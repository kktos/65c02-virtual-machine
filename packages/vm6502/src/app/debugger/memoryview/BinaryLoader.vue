	...
	<Sheet v-model:open="isLibraryOpen">
		...
		<SheetContent side="right" class="bg-gray-900 border-gray-700 text-gray-100 w-[400px] flex flex-col h-full overflow-hidden">
			...
			<ScrollArea class="mt-6 flex-1 min-h-0 pr-4">
				<div class="space-y-4">
					...
					<div
						...
						:key="bin.key.toString()"
						class="flex items-center justify-between p-3 bg-gray-800 rounded border border-gray-700"
					>
						<template v-if="editingKey === bin.key">
							<div class="flex-1 mr-2">
								<input
									:ref="
										(el) => {
											if (el) editInput = el as HTMLInputElement;
										}
									"
									v-model="editingName"
						...
						<template v-else>
							<div class="overflow-hidden mr-3">
								<div class="font-medium truncate text-sm text-gray-200" :title="bin.name">
									{{ bin.name }}
								</div>
			...
			<div class="mt-6 pt-6 border-t border-gray-800 shrink-0">
				<label
					class="flex items-center justify-center px-4 py-2 bg-gray-800 hover:bg-gray-700 text-gray-300 rounded border border-gray-600 cursor-pointer transition-colors"
				>
