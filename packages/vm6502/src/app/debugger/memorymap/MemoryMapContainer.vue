			...
			<div class="flex flex-col space-y-2 min-h-0">
				...
				<ul class="space-y-1 overflow-y-auto pr-2 -mr-2 flex-1">
					...
					>
						...
						<button v-if="region.removable !== false" @click="removeRegion(region.name)" class="text-red-500 hover:text-red-400 font-bold text-lg leading-none px-1">
							&times;
						</button>
					</li>
				</ul>
			...
			<div class="flex flex-col space-y-2">
				...
				<form @submit.prevent="addNewRegion" class="flex flex-col gap-3 p-4 bg-gray-900/50 rounded-lg border border-gray-700/50">
					<div class="flex items-center gap-3">
						...
						<input id="new-region-name" v-model="newRegion.name" placeholder="e.g. VRAM" class="flex-1 bg-gray-800 text-sm text-gray-200 border border-gray-600 rounded px-2 py-1.5 focus:outline-none focus:border-cyan-500" />
					</div>
					<div class="flex items-center gap-3">
						<label for="new-region-start" class="text-sm text-gray-400 w-16">Start</label>
						<input id="new-region-start" v-model="newRegion.start" placeholder="Hex, e.g. 400" class="flex-1 bg-gray-800 text-sm text-gray-200 border border-gray-600 rounded px-2 py-1.5 focus:outline-none focus:border-cyan-500 font-mono" />
					</div>
					<div class="flex items-center gap-3">
						<label for="new-region-size" class="text-sm text-gray-400 w-16">Size</label>
						<input id="new-region-size" v-model="newRegion.size" placeholder="Hex, e.g. 400" class="flex-1 bg-gray-800 text-sm text-gray-200 border border-gray-600 rounded px-2 py-1.5 focus:outline-none focus:border-cyan-500 font-mono" />
					</div>
					<div class="flex items-center gap-3">
						<label for="new-region-color" class="text-sm text-gray-400 w-16">Color</label>
						<input id="new-region-color" v-model="newRegion.color" type="color" class="h-8 w-12 p-0.5 bg-gray-800 border border-gray-600 rounded cursor-pointer" />
					</div>
					<button type="submit" class="mt-2 px-4 py-2 text-sm font-bold bg-cyan-600 hover:bg-cyan-500 text-white rounded-md w-full">Add Region</button>
				</form>
			</div>
		</div>
	</div>
