...
function subADC(oper) {
	...
	core.A = tempResult;
	if (core.A === 0) core.FlagZ = 1;
	else core.FlagZ = 0;
	if (core.A >= 0x80) core.FlagN = 1;
	else core.FlagN = 0;
}
...
function subASL(oper) {
	...
	if (result === 0) core.FlagZ = 1;
	else core.FlagZ = 0;
	if (result >= 0x80) core.FlagN = 1;
	else core.FlagN = 0;
...
function subBIT(oper) {
	if ((core.A & oper) === 0) core.FlagZ = 1;
	else core.FlagZ = 0;
	if (oper & 0x80) core.FlagN = 1;
	else core.FlagN = 0;
	if (oper & 0x40) core.FlagV = 1;
	else core.FlagV = 0;
}
function subCMP(oper) {
	let temp = core.A - oper;
	if (temp === 0) core.FlagZ = 1;
	else core.FlagZ = 0;
	if (temp >= 0) core.FlagC = 1;
	else core.FlagC = 0;
	if (temp < 0) temp += 0x100;
	if (temp >= 0x80) core.FlagN = 1;
	else core.FlagN = 0;
}
...
function subDEC(oper) {
	...
	if (newValue === 0) core.FlagZ = 1;
	else core.FlagZ = 0;
	if (newValue >= 0x80) core.FlagN = 1;
	else core.FlagN = 0;
...
function subLDX(oper) {
	core.X = oper;
	if (core.X === 0) core.FlagZ = 1;
	else core.FlagZ = 0;
	if (core.X >= 0x80) core.FlagN = 1;
	else core.FlagN = 0;
}
function subLDY(oper) {
	core.Y = oper;
	if (core.Y === 0) core.FlagZ = 1;
	else core.FlagZ = 0;
	if (core.Y >= 0x80) core.FlagN = 1;
	else core.FlagN = 0;
}
...
function opPLP() {
	...
	if (t0 & 1) core.FlagC = 1;
	else core.FlagC = 0;
	if (t0 & 2) core.FlagZ = 1;
	else core.FlagZ = 0;
	if (t0 & 4) core.FlagI = 1;
	else core.FlagI = 0;
	if (t0 & 8) core.FlagD = 1;
	else core.FlagD = 0;
	//if (t0&16) core.FlagB=1;else core.FlagB=0;
	core.FlagB = 1;
	if (t0 & 64) core.FlagV = 1;
	else core.FlagV = 0;
	if (t0 & 128) core.FlagN = 1;
	else core.FlagN = 0;
	core.cycle_count += 4;
}
...
function opBIT_IMMED() {
	//0x89
	if ((core.A & memIMMED()) === 0) core.FlagZ = 1;
	else core.FlagZ = 0;
	core.cycle_count += 2;
}
function opTXA() {
