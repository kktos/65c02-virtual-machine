...
export default class Disk {
	...
	async read(diskID, { track, sector, addr, length }) {
		if (!this.diskImages[diskID]) return;
		...
		console.log(
			"disk",
			`READ ${ts}[$${offset.toString(16)}] to $${addr.toString(16)}:$${(addr + length).toString(16)}`,
		);
	}
	...
	async readSector(diskID, track, sector, addr, isInterleaved) {
		...
		let sct = sector;
		if (isInterleaved) {
			sct = deInterleave[sct];
		}
		const offset = track * 0x1000 + sct * 0x100;
		const data = this.diskImages[diskID].slice(offset, offset + 0x0100);
		await this.vm.memWriteBin(0, addr, data);

		const ts = `T${track.toString(16)}:S${sct.toString(16)}`;
