function getAsmExpression(sym, ofs) {
	...
	while (matches) {
		...
		if (matches[1] === "ASC") {
			if (r.v.length > 1)
				return { error: "illegal quantity, string too long", pict: s.substring(0, strIdx) + r.v, et: ET_P };
		...
		if (mode === 0) {
			...
			if (c === "$") {
				...
				if (chunk === "STRING$(") {
					...
					const rv = getExpression(ctx, r.pict);
					...
					pict += `${rv.pict},`;
					const rs = getBBCBasicString(s.substring(i), ")");
					if (rs.error) {
						return { pict: pict + rs.pict, v: result, error: rs.error, et: rs.et };
					}
					pict += rs.pict;
					for (let k = 0, kn = Math.min(rv.v, 0xff); k < kn; k++) result += rs.v;
					i += rs.idx;
					hasContent = true;
					mode = 2;
					if (i < max && /^\s*[+-\/\*]\s*[&0123456789%@\(]/.test(s.substring(i))) break;
