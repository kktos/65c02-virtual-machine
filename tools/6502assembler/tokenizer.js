...
function getChar(ctx, isQuote) {
	if (ctx.srcLineIdx >= ctx.codesrc.length) return "EOF";
...
export function registerNextLineHandler(name, fn) {
	...
	if (nextLineHandlers) nextLineHandlers.prev = newHandler;
...
export function nextLine(ctx) {
	...
	while (handler) {
		if (handler.fn(ctx)) break;

		const prev = handler.prev;
		const next = handler.next;
		if (prev) prev.next = next;
		else nextLineHandlers = next;
