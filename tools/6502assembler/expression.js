...
const numberCharacters = [...digitCharacters, "$", "%", "@", "&", "'", '"'];
...
function getNumber(ctx, s, fromIdx, doubleWord) {
	const c0 = s.charAt(fromIdx);
	const size = doubleWord ? 0xffffffff : 0xffff;
	...
	switch (c0) {
		...
		case "&": {
			...
			const n = s.substring(fromIdx + 1, idx);
			const isWord = (n.length >= 4 && n.indexOf("00") === 0) || !!doubleWord;
			return { v: Number.parseInt(n, 16) & size, idx: idx, error: false, isWord: isWord };
		}
		...
		case "%": {
			...
			if (idx === fromIdx + 1) return { v: -1, idx: idx, error: true, et: ET_P };
			return { v: Number.parseInt(s.substring(fromIdx + 1, idx), 2) & size, idx: idx, error: false };
		}
		...
		case "@": {
			...
			return { v: Number.parseInt(s.substring(fromIdx + 1, idx), 8) & size, idx: idx, error: false };
		}
		...
		case "0": {
			if (s.length === fromIdx + 1) return { v: 0, idx: fromIdx + 1 };
			let ofs = fromIdx + 1;
			let base = 8;
			let c = s.charAt(ofs);
			if (c === "X") {
			...
			const n = s.substring(ofs, idx);
			const isWord = (base === 16 && n.length >= 4 && n.indexOf("00") === 0) || !!doubleWord;
			return {
				v: Number.parseInt(n, base) & size,
				idx: idx,
				error: false,
				isWord: isWord,
				lc: base !== 8 ? ofs - 1 : -1,
		...
		default: {
			...
			if (idx === fromIdx) return { v: -1, idx: idx, error: true };
			return { v: Number.parseInt(s.substring(fromIdx, idx), 10) & size, idx: idx, error: false };
		}
	}
}
	...
	function resolveOperation(pr) {
		...
		switch (op) {
			...
			case "*":
				...
				break;
			case "/": {
				if (pr === 0) return { v: -1, pict, error: "division by zero", et: ET_C };
				result /= pr;
				break;
			}
		...
		if (typeof result === "number") result &= size;
		op = "";
		return null;
	}
	...
	for (let i = 0; i < stack.length; i++) {
		...
		switch (item.type) {
			...
			case "fn": {
				...
				if ((err = resolveOperation(rez))) return err;
				break;
			}

			case "num":
				if (item.isWord && !mod) isWord = true;
				if ((err = resolveOperation(item.v))) return err;
				break;

			case "ident": {
				const sym = getNSentry(ctx, item.v);
				if (!sym) {
					if (ctx.pass === 1) return { v: mod ? 0xff : 0xffff, idx, pict, error: false, isWord: !mod, undef: item.v };

					return { v: -1, pict, error: true, isWord: true, undef: item.v, et: ET_C };
				}

				if (!mod && (sym.isWord || sym.pc > ctx.pc)) isWord = true;
				if ((err = resolveOperation(sym.v))) return err;
				break;
			}
			...
			case "paren": {
				...
				if ((err = resolveOperation(exp.v))) return err;
...
export function getExpression(ctx, s, doubleWord) {
			...
			default:
				if (numberCharacters.includes(c)) {
					...
					if (r.error) {
						...
						if (c === "'" && r.v >= 0) return { v: -1, pict, error: "illegal quantity", et: ET_P };
				...
				if (identifierCharacters.includes(c)) {
					...
					if (ctx.opcodes[r.v]) return { v: -1, pict, error: `illegal identifier (opcode ${r.v})`, et: ET_P };
	...
	function state2() {
		switch (c) {
			...
			case ")":
				lvl--;
				if (lvl < 0) return { v: -1, pict, error: 'non matching parenthesis "]"', et: ET_P };
				stack = parent[lvl];
	...
	if (state !== 2) return { v: -1, pict, error: "number or identifier expected", et: ET_P };
	if (lvl !== 0) return { v: -1, pict, error: 'non matching parenthesis, "]" expected.', et: ET_S };
