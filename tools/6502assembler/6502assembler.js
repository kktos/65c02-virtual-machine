...
let codeStore;
const bbcMode = false;
const redefSyms = false;
let bbcBlock = false;
let isHead;
let anonymousTargets;
...
function startAssembly(mainFilename, opts) {
	...
	ctx.options.listing = opts.listing === true;

	// ctx.charEncoding= encodeAscii;
	ctx.comment = "";
	ctx.readFile = (filename) => {
		const src = opts.readFile(filename);
		return src ? splitSrc(src) : null;
	};

	ctx.codesrc = ctx.readFile(mainFilename);
	ctx.filename = mainFilename;
	if (opts.segments) ctx.segments = opts.segments;
	else {
	...
	if (pass1) {
		...
		if (pass2) {
			...
			if (hasCode) ctx.listing += `\ndone (code: ${range}).`;
			else ctx.listing += "\ndone.\nno code generated.";
		}
	}

	if (ctx.options.listing) log(ctx.listing);
...
export function dumpCode(ctx, segmentName, wannaShowAddr) {
	...
	const ofs = wannaShowAddr ? codeStart % 8 : 0;

	for (let addr = codeStart - ofs; addr < codeEnd; addr++) {
		if (addr % 8 === 0 && wannaShowAddr) s += `${getHexWord(addr)}: `;
...
export function dumpSymbols() {
	...
	function dumpNSSymbols(NSsymbols) {
		...
		keys.sort().forEach((key) => {
			...
			symbols.push(
				` ${key.padStart(15, " ")} ${sym.isWord || sym.v > 0xff ? hexPrefix + getHexWord(sym.v) : `  ${hexPrefix}${getHexByte(sym.v)}`}`,
			);
		});
		return symbols;
	}
...
function asmPass(ctx) {
	...
	while (ctx.sym) {
		...
		if (isIdentifier) {
			...
			if (ctx.pass === 1) {
				if (r.idx !== identRaw.length) {
					const parsed = identRaw.substring(0, r.idx);
					const illegalChar = identRaw.charAt(r.idx);
					let message = `Illegal character "${illegalChar}"`;
			...
			if (ctx.sym.length > 1 && (ctx.sym[ctx.ofs] === "=" || ctx.sym[ctx.ofs] === "EQU")) {
				...
				if (ctx.sym.length > ctx.ofs) {
					if (ctx.sym.length === ctx.ofs + 1 && ctx.sym[ctx.ofs] === "W") {
						// ignore 'W' suffix
			...
			if (ctx.macros[ident]) {
				if (expandMacro(ctx, ident)) continue;
		...
		if (c0 < "A" || c0 > "Z") {
			...
			// else
			if (ctx.ofs > 0) logError(ctx, ET_S, "character or assignment operator expected");
		...
		const dot = ctx.sym[ctx.ofs].indexOf(".");
		let ext = "";
		let opctab;
		let instr;
		let addr;
		...
		if (typeof addr === "undefined") {
			...
			if (
				...
				const target = getAnonymousTarget(addr);
				if (target.error) {
				...
				const expr = addr.substring(b1, b2);
				const e0 = expr.charAt(0);
				let autoZpg = ctx.options.autoZpg && !ext && mode >= 3 && mode <= 5 && hasZpgMode(opc);
				...
				const r = getExpression(ctx, expr);
				if (r.error) {
					ctx.pict += r.pict;
					if (r.undef) {
						logError(ctx, r.et || ET_C, `undefined symbol "${r.undef}"`);
				...
				if (ctx.pass === 1) {
					...
					ctx.pict += hexPrefix + (steptab[mode] > 2 ? getHexWord(oper) : getHexByte(oper));
				}
				ctx.pict += coda;
			}
